...
class Table:
    def __init__(self, name, columns, primary_key=None, unique_keys=None):
        ...
        #Indexing for primary key and unique keys
        self.indexes = {}
        if self.primary_key:
    ...
    # columns existence and validate types
        ...
        if pk_value in self.indexes[self.primary_key]:
            raise ValueError(
                f"Primary key constraint violated: {self.primary_key}={pk_value}"
            )
    ...
    # Enforce unique keys
        ...
        if value in self.indexes[key]:
            raise ValueError(
                f"Unique constraint violated: {key}={value}"
            )
